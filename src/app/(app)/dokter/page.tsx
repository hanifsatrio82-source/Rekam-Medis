                        ...
                        <div key={v.id} className="card" style={{padding:0,overflow:"hidden"}}>
                          ...
                          {rmExpanded[v.id] && (
                            <div style={{padding:"var(--s4)",display:"flex",flexDirection:"column",gap:"var(--s3)",fontSize:".875rem"}}>
                              <div><div style={{fontWeight: 700,letterSpacing:".06em",color:"var(--blue)",fontSize:".7rem",textTransform:"uppercase",marginBottom:4}}>S — Subjektif</div><div><span style={{color:"var(--text-4)"}}>Keluhan:</span> <strong style={{color:"var(--text)"}}>{fmt(cn?.keluhan_utama)}</strong></div><div><span style={{color:"var(--text-4)"}}>Riwayat Sekarang:</span> <span style={{color:"var(--text-2)"}}>{fmt(cn?.riwayat_penyakit_sekarang)}</span></div><div><span style={{color:"var(--text-4)"}}>Catatan:</span> <span style={{color:"var(--text-2)"}}>{fmt(cn?.catatan_subjektif)}</span></div></div>
                              <div><div style={{fontWeight: 700,letterSpacing:".06em",color:"var(--teal)",fontSize:".7rem",textTransform:"uppercase",marginBottom:4}}>O — Objektif</div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>{[["TD", cn?.tekanan_darah], ["Suhu", cn?.suhu ? `${cn.suhu}°C` : "—"], ["SpO₂", cn?.spo2 ? `${cn.spo2}%` : "—"], ["HR", fmt(cn?.hr)], ["RR", fmt(cn?.rr)], ["TB", cn?.tb ? `${cn.tb} cm` : "—"], ["BB", cn?.bb ? `${cn.bb} kg` : "—"], ["Draft", cn ? (cn.is_draft ? "Ya" : "Final") : "—"]].map(([label, value]) => (<div key={label} style={{background:"var(--surface-2)",padding:6,borderRadius:8,textAlign:"center"}}><div style={{fontSize:".6rem",color:"var(--text-4)"}}>{label}</div><div style={{fontWeight:700,fontSize:".78rem"}}>{value}</div></div>))}</div><div style={{marginTop:6}}><span style={{color:"var(--text-4)"}}>Pemeriksaan Fisik:</span> <span style={{color:"var(--text-2)"}}>{fmt(cn?.catatan_pemeriksaan_fisik)}</span></div></div>
                              <div><div style={{fontWeight: 700,letterSpacing:".06em",color:"var(--violet)",fontSize:".7rem",textTransform:"uppercase",marginBottom:4}}>A — Assessment</div><div style={{color:"var(--text-2)"}}>{cn?.edukasi ? <span><strong>Edukasi/Plan:</strong> {cn.edukasi}</span> : <span style={{color:"var(--text-4)"}}>— Tidak ada catatan assessment —</span>}</div></div>
                              <div><div style={{fontWeight: 700,letterSpacing:".06em",color:"var(--green)",fontSize:".7rem",textTransform:"uppercase",marginBottom:4}}>P — Plan</div>{v.visit_actions?.length > 0 ? <div><span style={{color:"var(--text-4)"}}>Tindakan:</span> {v.visit_actions.map((a: any) => `${a.actions?.nama || a.fee_snapshot_nama} (${formatRupiah(a.fee_snapshot_tarif)})`).join(", ")}</div> : null}{v.prescriptions?.filter((p: any) => p.is_active).length > 0 ? <div><span style={{color:"var(--text-4)"}}>Resep:</span> {v.prescriptions.filter((p: any) => p.is_active).flatMap((p: any) => (p.prescription_items || []).map((it: any) => `${it.medications?.nama} ${it.medications?.kekuatan || ""} x${it.jumlah} (${it.aturan_pakai})`)).join(" · ") || "—"}</div> : <div><span style={{color:"var(--text-4)"}}>Resep:</span> —</div>}{v.powders?.filter((p: any) => p.is_active).length > 0 ? <div><span style={{color:"var(--text-4)"}}>Puyer:</span> {v.powders.filter((p: any) => p.is_active).map((p: any) => `${p.jumlah_bungkus} bungkus (${p.aturan_pakai})`).join(", ")}</div> : null}{cn?.fee_snapshot_nama ? <div><span style={{color:"var(--text-4)"}}>Biaya:</span> {cn.fee_snapshot_nama} — {formatRupiah(cn.fee_snapshot_tarif)}</div> : null}</div>
                            </div>
                          )}
                        </div>
                      );
