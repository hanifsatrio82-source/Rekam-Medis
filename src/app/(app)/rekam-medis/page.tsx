                    ...
                    <div key={v.id} className="card" style={{padding:0,overflow:"hidden"}}>
                      ...
                      {expanded[v.id] && (
                        <div style={{padding:"var(--s4)",display:"flex",flexDirection:"column",gap:"var(--s4)"}}>
                          {/* S — Subjektif (mengikuti form dokter) */}
                          <div>
                          ...
                          {/* O — Objektif / TTV (mengikuti form dokter) */}
                          <div>
                            ...
                            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                              {[
                                ["TD", cn?.tekanan_darah || "—"],
                                ["Suhu", cn?.suhu ? `${cn.suhu}°C` : "—"],
                                ["SpO₂", cn?.spo2 ? `${cn.spo2}%` : "—"],
                                ["HR", fmt(cn?.hr)],
                                ["RR", fmt(cn?.rr)],
                                ["TB", cn?.tb ? `${cn.tb} cm` : "—"],
                                ["BB", cn?.bb ? `${cn.bb} kg` : "—"],
                                ["BMI", (cn?.tb && cn?.bb) ? ((Number(cn.bb) / ((Number(cn.tb) / 100) ** 2)).toFixed(1)) : "—"],
                              ].map(([label, value]) => (
                          ...
                          <div>
                            ...
                            {hasSOAP ? (() => {
                              let dx: any[] = [];
                              try { dx = cn?.riwayat_penyakit_sekarang ? JSON.parse(cn.riwayat_penyakit_sekarang) : []; } catch { dx = []; }
                              return dx.length > 0 ? (
                                <div style={{display:"flex",flexDirection:"column",gap:4,fontSize:".875rem"}}>
                                  {dx.map((d: any, i: number) => (
                                    <div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"2px 0"}}><span style={{fontSize:".68rem",fontWeight:700,color:"var(--text-4)",background:"var(--surface-2)",padding:"1px 6px",borderRadius:4}}>{d.kode || "—"}</span><span style={{color:"var(--text)"}}>{d.nama}</span></div>
                                  ))}
                                </div>
                              ) : <span style={{color:"var(--text-4)",fontSize:".875rem"}}>— Tidak ada diagnosis —</span>;
                            })() : <span style={{color:"var(--text-4)",fontSize:".875rem"}}>— Belum diisi —</span>}
                          </div>
                          {/* P — Plan */}
                          <div>
                            ...
                            <div style={{display:"flex",flexDirection:"column",gap:6,fontSize:".875rem"}}>
                              <div><span style={{color:"var(--text-4)"}}>Edukasi / Rencana Tindak Lanjut:</span> <span style={{color:"var(--text-2)"}}>{fmt(cn?.edukasi)}</span></div>
                              {v.visit_actions?.length > 0 ? <div><span style={{color:"var(--text-4)"}}>Tindakan:</span> {v.visit_actions.map((a: any) => `${a.actions?.nama || a.fee_snapshot_nama}`).join(", ")}</div> : <div><span style={{color:"var(--text-4)"}}>Tindakan:</span> —</div>}
