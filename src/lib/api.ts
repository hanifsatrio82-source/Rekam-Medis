...
const VISIT_SELECT = `
  ...
  patients(id, no_rm, nama, tanggal_lahir, jenis_kelamin, alamat, no_hp),
...
export async function getAkunSaya() {
  ...
  const { data: profile } = await supabase.from("profiles").select("*").eq("id", user.id).single().then(r=>r as any).catch(()=>({data:null}));
